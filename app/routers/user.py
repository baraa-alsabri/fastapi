from .. import utls, models, schemas
...
@router.post(
    "/", status_code=status.HTTP_201_CREATED, response_model=schemas.UserResponseModel
)
def create_user(user: schemas.UserModel, db: Session = Depends(get_db)):
    hashed_password = utls.hash_password(user.password)
